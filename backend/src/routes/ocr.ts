...
import { parseDocumentImage, type SupportedImageMediaType } from '../services/claudeOcr.js';
 ...
 */
router.post('/parse', requireAuth, upload.single('photo'), async (req: AuthedRequest, res) => {
  try {
    if (!req.file) {
    ...
    const mediaType = req.file.mimetype as SupportedImageMediaType;
