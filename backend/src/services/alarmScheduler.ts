...
async function dispatchAlarm(supabase: ReturnType<typeof getServiceClient>, rule: DueAlarmRule) {
  ...
  const phoneNumbers = [...new Set(allRecipients.map((r) => r.phone_number).filter(Boolean))] as string[];
...
async function runDueAlarmsCheck() {
  ...
  for (const rule of (dueRules ?? []) as unknown as DueAlarmRule[]) {
