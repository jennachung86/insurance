...
// Claude 비전이 받는 이미지 media_type은 이 4종만 지원한다 (HEIC/HEIF 불가).
// 모바일에서 카메라로 찍은 사진이 HEIC인 경우 업로드 전 JPEG로 변환해야 한다 -
// PhotoAttachButton.tsx는 expo-image-picker의 기본 JPEG 출력을 사용하므로 문제 없다.
export type SupportedImageMediaType = 'image/jpeg' | 'image/png' | 'image/webp' | 'image/gif';

interface ParseImageInput {
  base64Data: string;
...
export async function parseDocumentImage({
  ...
  const response = await client.beta.messages.create({
    ...
    max_tokens: 2048,
    system:
      '너는 보험 증권, 검사 확인서, 납입 영수증 사진에서 관리용 일정 데이터를 추출하는 어시스턴트다. ' +
